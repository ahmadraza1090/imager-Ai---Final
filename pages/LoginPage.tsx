...
const authImages = [
  ...
  'https://images.unsplash.com/photo-1605332152013-0bb69a1f7a4e?q=80&w=1200&auto=format&fit=crop',
];
...
// Fix: Defined LoginPageProps interface to resolve 'Cannot find name' error.
...
const LoginPage: React.FC<LoginPageProps> = ({ navigate }) => {
  ...
  const handleSocialLogin = (provider: 'google' | 'github') => {
    ...
    login(demoEmail, 'password'); // Use a dummy password for simulation
    navigate('generator');
  };
      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
        <div className="mx-auto w-full max-w-sm lg:w-96">
          <div>
            ...
            <h2 className="mt-6 text-3xl font-extrabold text-gray-900 dark:text-white">Log in to your account</h2>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Or{' '}
              <button onClick={() => navigate('signup')} className="font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300">
            ...
            <div className="mt-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1">
                  <label htmlFor="password-for-login" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Password</label>
                  <div className="mt-1 relative">
                    <input id="password-for-login" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="appearance-none block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-white/50 dark:bg-gray-800/50" />

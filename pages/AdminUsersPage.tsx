    ...
    return (
        <div className="py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <input type="text" placeholder="Search users by name or email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="mb-6 w-full md:w-1/2 px-4 py-2 bg-white/50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                                ...
                                {filteredUsers.map(user => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                                            <button onClick={() => { setEditingUser(user); setCredits(user.credits); }} className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors">Edit Credits</button>
                ...
                <Modal isOpen={!!editingUser} onClose={() => setEditingUser(null)} title={`Edit Credits for ${editingUser?.name}`}>
                    <div className="space-y-4">
                        ...
                        <input type="number" id="credits" value={credits} onChange={e => setCredits(parseInt(e.target.value, 10) || 0)} className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500" />
                        <div className="flex justify-end space-x-2 pt-2">
                            ...
                            <motion.button onClick={handleEditCredits} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 shadow" whileHover={{scale: 1.05}} whileTap={{scale:0.95}}>Save</motion.button>
                        </div>
                    </div>
                </Modal>
